...
public class Account {
    ...
    public void deposit(double amount) throws Exception {
        try {
            if (amount < 0) {
                throw new Exception("Negative amount error"); //is checked
            }
            balance += amount;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * Withdraws an amount of money from account based on a valid ssn
     * @param amount , the amount of money to be withdrawn
     * @param ssn , the given ssn
     * @throws Exception , if account's balance not enough or ssn invalid
     */

    public void withdraw(double amount, String ssn) throws Exception {
        try {
            if (amount > balance) {
                throw new Exception("Insufficient balance error"); //is checked
            }
